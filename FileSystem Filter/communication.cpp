...
NTSTATUS Communicate::StartCommunicate(PFLT_FILTER filter,
	PFLT_PORT port,
...
void Communicate::CloseCommunication(PFLT_PORT port) {

	FltCloseCommunicationPort(port);

}

NTSTATUS Communicate::DataSend(PFLT_FILTER filterHandle,PFLT_PORT clientPort, PVOID buffer, ULONG bufferSize, PVOID response, ULONG responseSize, ULONG timeOut) {

	LARGE_INTEGER convertedTimeOut;
	convertedTimeOut.QuadPart = -static_cast<INT64>(timeOut) * 10 * 1000;
